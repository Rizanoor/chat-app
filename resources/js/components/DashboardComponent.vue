    ...
    <div class="flex flex-col h-[66vh] overflow-y-auto px-6">
        <div class="justify-center px-4">
            <h1 class="text-left text-xl font-bold text-[#5E296B]">Your Matches <span
                    class="text-[#E4ACD4] font-bold">{{ users.length }}</span></h1>
        </div>
            ...
            <div class="grid grid-cols-2 gap-4 justify-items-center">
                <a v-for="user in users" :key="user.id" :href="`/chat/${user.id}`">
                    <div class="bg-white rounded-xl shadow-md border border-[#E4ACD4] p-3 w-48 relative">
                        ...
                        <img class="rounded-lg w-full h-40 object-cover" :src="`https://i.pravatar.cc/20${user.id}`" :alt="user.name">
                        <div class="mt-4 text-center">
                            <div
                                class="text-sm text-gray-800 bg-gray-500 bg-opacity-20 backdrop-blur-md rounded-full px-2 py-1 inline-block mb-2 border border-white">
                                3.3 km away
                            </div>
                            <h2 class="text-lg font-bold text-gray-900">{{ user.name }}, 20</h2>
